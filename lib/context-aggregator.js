...
ContextAggregator.prototype.updatePredicate = function(data){
  ...
  if(process.env.DEBUG_APP !== "F"){
    console.log("Predicate Updating (provider, predicate):", provider, predicate);
  }
  ...
  if(process.env.DEBUG_APP !== "F"){
    console.log("Predicate Changed(provider, predicate):", provider, predicate);
  }
...
ContextAggregator.prototype._predicateChanged = function(provider, predicate, groundings){
  ...
  satisifiedRules.map(function(rule){
    if(process.env.DEBUG_APP !== "F"){
      console.log("Rule Invoked(rule, provider, predicate):", rule.id, provider, predicate);
    }
...
ContextAggregator.prototype.getRulesByInputPPG = function(provider, predicate, groundings){
  ...
  */
  var out = [];
  for(var i in rules){
    for(var j in rules[i].inputContexts){
      if(rules[i].inputContexts[j].provider !== provider || rules[i].inputContexts[j].predicate.raw !== predicate){
        continue;
      }
      for(var k = 0; k < rules[i].inputContexts[j].groundings.length; k++){
        if(rules[i].inputContexts[j].groundings[k] !== groundings[k] && rules[i].inputContexts[j].groundings[k] !== "_"){
          break;
        }
      }
      if(k === rules[i].inputContexts[j].groundings.length){
        rules[i].id = i;
        out.push(rules[i]);
      }
    }
